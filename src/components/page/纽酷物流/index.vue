<template>
  <div class="p-page">
    ...
    >
      <template #userInfo>
        <div :class="'header' + (cardId + 1)" @click="show = true">
          ...
          <div class="pm">
            <div class="text4">
              <van-image :src="IMG + 'nt/icon/phone.png'" />{{ userInfo.phone }}
            </div>
            <div class="text5">
              <van-image :src="IMG + 'nt/icon/email.png'" />{{ userInfo.email }}
            </div>
            <div class="text6">
              <van-image :src="IMG + 'nt/icon/address.png'" />
    ...
    <!-- 电话和地址 -->
    <pAddress
      phoneButton="nt/icon/phoneButton.png"
      addressButton="nt/icon/addressButton.png"
    ...
    <pQrcode
      ...
      qrCodeIcon="nt/icon/sys.png"
      :isXx="userInfo.image"
      phonoIcon="nt/icon/grxx.png"
    ></pQrcode>
    <!-- tab栏 -->
    <tab
      :tabList="tabList"
    ...
    <!-- 公司介绍 -->
    <headerContent
      headerImg="nt/headerImage/gsjs.png"
      width="638rpx"
      height="46rpx"
      id="gsjs"
    >
      <template #content>
        <van-image :src="IMG + 'nt/image/jjBg.png'" class="m-t-14" />
        <p
          class="textStyle"
    ...
    <!-- 企业优势 -->
    <headerContent
      headerImg="nt/headerImage/qyys.png"
      width="638rpx"
      height="46rpx"
    >
      <template #content>
        <div class="ysBox">
          <div
            v-for="(item, index) in getContent('企业优势').solution_list"
            class="ysItem"
            :key="index"
          >
            <van-image
    ...
    <!-- 产品服务 -->
    <headerContent
      headerImg="nt/headerImage/cpfw.png"
      width="100%"
      height="23px"
      id="cpfw"
    >
      <template #content>
        ...
        <listBox
          v-for="(item, index) in getContent('产品服务').solution_list"
          :key="index"
          v-if="item"
          :type="1"
          :imageUrl="{
     ...
     <!-- 联系我们 -->
     <headerContent
      headerImg="nt/headerImage/lxwm.png"
      width="100%"
      height="23px"
      id="lxwm"
    >
      <template #content>
        <companyTitle title="前海纽酷（深圳）国际物流供应链管理有限公司" fontSize="14px"></companyTitle>
        <companyTitle title="电 话：189-2744-3847" color="#666666"></companyTitle>
        <companyTitle title="邮 箱：niukuguoji@usniuku.com" color="#666666"></companyTitle>
        <companyTitle title="官 网：http://www.usniuku.com" color="#666666"></companyTitle>
        <companyTitle title="地 址：" color="#666666" ></companyTitle>

        <div
          v-for="(item, index) in getContent('地址列表').solution_list"
          :key="index" >
          <van-image @click="getMap(item.describe,item.name)" :src="item.image.url" v-if="item.image" style="margin-top:8px"></van-image>
          <div v-else @click="getMap(item.describe,item.name)" class="lxBox">
            <div class="line"></div>
            {{ item.name }}
          </div>
        </div>
