<template>
  <div class="p-page">
      ...
      <template #userInfo>
        <div :class="'header' + (cardId + 1)" @click="show = true">
          ...
          <div class="pm">
            ...
            <div class="text4">
              <van-image :src="IMG + 'ddzn/icon/phone.png'" />{{
                userInfo.phone || "-"
              }}
            </div>
            <div class="text5">
              <van-image :src="IMG + 'ddzn/icon/email.png'" />{{
                userInfo.email || "-"
              }}
            </div>
            <div class="text6">
              <van-image :src="IMG + 'ddzn/icon/address.png'" />
              <div class="textccc">
                {{ userInfo.address || "浙江省宁波市江北区风华路201号" }}
              </div>
            </div>
          </div>
          <van-image v-if="userInfo.wx_code" width="57px" height="57px" :src="userInfo.wx_code.url"></van-image>
          </div>
        </div>
      </template>
      <template #default="slotProps">
        ...
        >
          ...
          <div class="pm">
            ...
            <div class="text5">
              <van-image :src="IMG + 'ddzn/icon/ema li.png'" />{{
                userInfo.email || "-"
              }}
            </div>
            <div class="text6">
    ...
    <!-- 电话和地址 -->
    <pAddress
      phoneButton="ddzn/icon/phoneButton.png"
      addressButton="ddzn/icon/addressButton.png"
      :phone="userInfo.phone"
      :address="userInfo.address || '浙江省宁波市高新区深蓝大厦503'"
      name="叮当智能"
      :type="1"
      backgroundColor="#193980"
      width="24px"
      height="24px"
    ></pAddress>
    <!-- 二维码和形象照 -->
    <pQrcode :isWx="userInfo.wx_code" qrCodeIcon="ddzn/icon/sys.png"></pQrcode>
    <!-- tab栏 -->
    <tab
      :tabList="tabList"
      :tabId="tabId"
      ...
      color="#193980"
    ></tab>
      <ddznContent ref="content" :list="list"></ddznContent>
    <!-- 阿库标识 -->
    <aKuLogo></aKuLogo>
  </div>
</template>
...
.selectCard {
  border: 1px solid #193980 !important;
  border-radius: 8px;
}
...
.jxBox {
  margin: 5px 0;
  border: 1px solid rgba($color: #193980, $alpha: 0.3);
  padding: 8px;
  display: flex;
  align-content: center;
  border-radius: 4px;
  .jsIcon {
    background-image: url("https://card-img1.oss-cn-beijing.aliyuncs.com/ddzn/icon/jxImage.png");
    width: 20px;
    height: 18px;
