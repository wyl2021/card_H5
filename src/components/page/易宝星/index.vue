<template>
  <div class="p-page">
    ...
    >
            ...
            <div>
              ...
              <div class="pm">
                <div v-if="userInfo.phone" class="text4">
                  <van-image :src="IMG + 'ybx/icon/phone.png'" />
                  {{ userInfo.phone }}
                </div>
                <div v-if="userInfo.email" class="text5">
                  <van-image :src="IMG + 'ybx/icon/email.png'" />{{
                    userInfo.email
                  }}
                </div>
                <div v-if="userInfo.address" class="text6">
                  <van-image :src="IMG + 'ybx/icon/address.png'" />
                  <div>
                    <div class="textccc">
                      {{ userInfo.address }}
                    </div>
      ...
      <!-- <template #cardBg>
        <div
          ...
          :style="{
            'background-image':
              'url(' + IMG + 'ybx/cardBg/cardBg' + (cardId + 1) + '.png)',
          }"
        ></div>
      </template> -->
      <template #default="slotProps">
        ...
        >
          ...
          <div class="pm">
            <div v-if="userInfo.phone" class="text4">
              <van-image
                :src="IMG + 'ybx/icon/phone' + (slotProps.item + 1) + '.png'"
              />
              {{ userInfo.phone }}
            </div>
            <div v-if="userInfo.email" class="text5">
              <van-image
                :src="IMG + 'ybx/icon/email' + (slotProps.item + 1) + '.png'"
              />{{ userInfo.email }}
            </div>
            <!-- <div v-if="userInfo.address" class="text6">
              <van-image
                v-if="slotProps.item + 1 != 1"
                :src="IMG + 'ybx/icon/address' + (slotProps.item + 1) + '.png'"
              />
              <div>
                <div
                  class="textccc"
    ...
    <!-- 电话和地址 -->
    <pAddress
      phoneButton="ybx/icon/phoneButton.png"
      addressButton="ybx/icon/addressButton.png"
    ...
    <pQrcode
      ...
      qrCodeIcon="ybx/icon/sys.png"
      :isXx="userInfo.image"
      phonoIcon="ybx/icon/grxx.png"
    ></pQrcode>
    <!-- tab栏 -->
    <tab
      :tabList="tabList"
    ...
    <!-- 公司介绍 -->
    <headerContent
      headerImg="ybx/headerImage/gsjj.png"
      width="100%"
      height="41px"
      id="gsjj"
    >
      <template #content>
        <van-image :src="IMG + 'ybx/image/jjBg.png'" class="m-t-14" />
        <p
          class="textStyle"
    ...
    <!-- 感恩伙伴 -->
    <headerContent
      headerImg="ybx/headerImage/gehb.png"
      width="100%"
      height="41px"
    >
      <template #content>
    ...
    <!-- 业务展示 -->
    <headerContent
      headerImg="ybx/headerImage/ywzs.png"
      width="135px"
      height="33px"
      id="ywzs"
    ></headerContent>
    ...
    <!-- 案例展示 -->
    <headerContent
      headerImg="ybx/headerImage/alzs.png"
      width="100%"
      height="41px"
      id="alzs"
    >
      <template #content>
        ...
        >
          <div class="alBg">
            ...
            <img :src="IMG + 'ybx/icon/right2.png'" />
          </div>
        </div>
    ...
    <!-- 联系我们 -->
    <headerContent
      headerImg="ybx/headerImage/lxwm.png"
      width="100%"
      height="41px"
      id="lxwm"
    >
      <template #content>
        ...
        <div v-for="(item, index) in getContent('联系我们')" :key="index">
          <div @click="getClick(item)" class="lxBox">
            ...
            <div class="describe">
              <img :src="IMG + 'ybx/icon/name.png'" />{{ item.name_en }}
            </div>
            <div class="describe">
              <img :src="IMG + 'ybx/icon/email.png'" />{{ item.pdf }}
            </div>
            <div class="describe">
              <img :src="IMG + 'ybx/icon/address.png'" />
...
export default {
  ...
  data() {
    return {
      ...
      popupData: null,
      textList: [],
      text: '易通科技物流集团创立于2012年，注册资金2000万元，总部位于粤港澳大湾区核心城市——深圳，公司依托工程项目，在香港、大连、天津、青岛、上海、宁波、嘉兴、无锡、福州、厦门、中山、东莞等国内主要口岸城市均设有分公司；为了完善全球网络布局，为客户提供更优质的物流服务，公司在美国洛杉矶，新加坡，越南胡志明，意大利米兰均设有子公司(办事处);为客户提供海运进出口，空运进出口，大件运输等物流服务。 依靠强大的专业团队以及仓储资源，公司自2016年开始进军亚马逊FBA电商物流，先后在东莞、嘉兴、义乌、福州、龙岩、青岛等设立仓储中心，并在美国、意大利设立海外分拨基地。目前FBA电商物流运输范围涵盖“美国、加拿大、欧洲、英国、墨西哥等全境"等多个国家，可为客户提供美森快船、COSCO快船、长荣定提、以星快船、空派、中欧海运运输、铁路运输、卡航运输、国际快递等全方位的亚马逊物流服务。 自营美西、美东海外仓，配备专业的华人操作团队，致力于为TK、TEMU、EBAY、速卖通等平台的卖家提供一件代发的卓越服务。我们承诺在24小时内完成操作指令，确保48小时内快递即可提取派送。此外，我们还提供移仓换标、大货中转、退货维修等一系列全面的库内操作服务，致力于为您打造全方位的物流体验，让您的跨境交易更加顺畅无忧。 易通科技，引领全球。易通全体同仁将始终秉持创新、拼搏、共赢的企业精神，让每一位客户都能乐享快捷、安全的优质服务。',
    };
  },
  created() {
    this.getUserInfo();
